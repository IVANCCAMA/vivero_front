...
function FormTransacciones() {
    ...
    const handleSubmit = async (values, { setSubmitting }) => {
        ...
        if (usuario.nombre_usuario ||usuario.id_tipo_transaccion|| usuario.ci_usuario || usuario.celular_usuario 
    ...
    const validationSchema = yup.object().shape({
        id_tipo_transaccion:yup.number().required("Campo obligatorio"),
        ...
        {({ handleSubmit, handleChange, values, touched, errors }) => (
            ...
            <Form.Group as={Row} className="mb-2" controlId="validationFormik07">
                ...
                <Form.Select
                    ...
                    onChange={handleChange}
                    isInvalid={touched.id_tipo_transaccion && !!errors.id_tipo_transaccion}
                    style={{ backgroundColor: '#A4BE7B' }}
                >
                    <option value="">Seleccionar</option>
                    {/* {tipodeUsuarios.map(usuarioT =>(
                    <option key={usuarioT.id_tipo_transaccion} value={usuarioT.id_tipo_transaccion}>
